...
class ButtonPad extends React.Component {
	...
	renderButton (letter) {
		...
		return (
			<button className={cls} key={letter} onClick={() => this.handleButtonClick(letter)}>
				{letter}
			</button>
		)
	}
	...
	render () {
		...
		this.props.targetWord.split('').forEach((letter, index) => {
			...
			const show =
					...
					this.state.status === 'Game Over' ? true :
					letterMap.show
...
class Hangman extends React.Component {
	...
	componentDidMount () {
		fetch(
			'https://api.wordnik.com/v4/words.json/randomWords?hasDictionaryDef=true&maxCorpusCount=-1&minDictionaryCount=1&maxDictionaryCount=-1&minLength=5&maxLength=-1&limit=1&api_key=c23b746d074135dc9500c0a61300a3cb7647e53ec2b9b658e'
		)
	...
	render () {
		return (
			<div>
				...
				<div>
					{
						this.state.targetWord === '' ? <p>Loading...</p> :
